...
case prefix+'infogrup':
            ...
            case prefix+'promote':
                ...
                if (mentioned.length !== 0){
                    ...
                    conn.groupMakeAdmin(from, [quotedMsg.sender])
                    .then((res) => reply(jsonformat(res)))
                    .catch((err) => reply(jsonformat(err)))
                } else if (!isNaN(args[1])) {
                    conn.groupMakeAdmin(from, [args[1] + '@s.whatsapp.net'])
                    .then((res) => reply(jsonformat(res)))
                    .catch((err) => reply(jsonformat(err)))
                } else {
            ...
            case prefix+'demote':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender === ownerNumber) return reply(`Tidak bisa kick Owner`)
                    conn.groupDemoteAdmin(from, [quotedMsg.sender])
                    .then((res) => reply(jsonformat(res)))
                    .catch((err) => reply(jsonformat(err)))
                } else if (!isNaN(args[1])) {
            ...
            case prefix+'sider': case prefix+'chatinfo':
                ...
                if (!quotedMsg.fromMe) return reply(`Reply pesan dari bot`)
                    conn.messageInfo(from, msg.message.extendedTextMessage.contextInfo.stanzaId)
                    .then((res) => {
                        let anu = []
                        let txt = `*Info Chat*\n\n`
            ...
            case prefix+'closegrup':
                ...
                .catch((err) => reply(jsonformat(err)))
                break
            case prefix+'setppgrup': case prefix+'tutup':
                if (!isGroup) return reply(mess.OnlyGrup)
                if (!isGroupAdmins && !isOwner)return reply(mess.GrupAdmin)
                if (!isBotGroupAdmins) return reply(mess.BotAdmin)
                if (isImage || isQuotedImage) {
